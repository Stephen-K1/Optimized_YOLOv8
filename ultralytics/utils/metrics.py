...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, gt_bboxes, gt_cls):
        ...
        detections = detections[detections[:, 4] > self.conf]  # (300, 6) -> (11, 6), self.conf=0.25
        gt_classes = gt_cls.int()
        detection_classes = detections[:, 5].int()
        ...
        if x[0].shape[0]:
            ...
            if x[0].shape[0] > 1:
                ...
                matches = matches[np.unique(matches[:, 1], return_index=True)[1]]   # 每个预测box只能匹配一个gt，如果匹配了多个gt，删掉iou小的预测box
                matches = matches[matches[:, 2].argsort()[::-1]]
                matches = matches[np.unique(matches[:, 0], return_index=True)[1]]   # 每个gt只能匹配一个预测box，如果匹配了多个预测box，删掉iou小的gt
        else:
