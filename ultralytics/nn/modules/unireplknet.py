...
class SEBlock(nn.Module):
    ...
    def forward(self, inputs):    # inputs.shape=(1,80,160,160)
        x = F.adaptive_avg_pool2d(inputs, output_size=(1, 1))   # x.shape=(1,80,1,1)
        x = self.down(x)   # x.shape=(1,20,1,1), self.down=Conv2d(80,20,kernel_size=1,stride=1)
        x = self.nonlinear(x)  # self.nonlinear=ReLU
        x = self.up(x)   # x.shape=(1,80,1,1), self.down=Conv2d(20,80,kernel_size=1,stride=1)
        x = F.sigmoid(x)
        return inputs * x.view(-1, self.input_channels, 1, 1)  # shape(1,80,160,160) * shape(1, 80, 1, 1)
...
class UniRepLKNetBlock(nn.Module):
    ...
    def compute_residual(self, x):
        ...
        y = self.pwconv2(self.act(self.pwconv1(y)))  # pwconv1: BCHW2BHWC (achieve by permute) + linear(80, 320)
...
class UniRepLKNet(nn.Module):
    ...
    def __init__(self,
        ...
        # self.for_pretrain = init_cfg is None   # original
        self.for_pretrain = init_cfg is not None    # 因为不改输入参数，原来这个是按照分类来初始化的，加个not，按照detection来初始化
        ...
        if self.for_pretrain:
            ...
            norm_layer = partial(LayerNorm, eps=1e-6, data_format="channels_first")  # partial()将某个方法或者类的一些参数固定了，得到一个新的方法或者类
...
# This decorator is used to register custom model architectures within the `timm` framework. 
# By registering a model, you make it accessible through the `timm.create_model()` function
@register_model
...
# define two modules for building UniRepLKNet via YOLOv8.yaml
...
class stage(nn.Module):
    def __init__(self, in_channels, channels, kernel_sizes):   #　原来不需要in_channels这个参数，因为YOLOv8构建模型默认传进in_channels，这里多加这个参数
        super().__init__()
        self.layers = nn.Sequential(
            *[UniRepLKNetBlock(dim=channels, kernel_size=kernel_sizes[i], drop_path=0.0,
                                layer_scale_init_value=1e-6, deploy=False,
                                attempt_use_lk_impl=False,
                                with_cp=False, use_sync_bn=False) for i in range(len(kernel_sizes))]
        )
...
class normConcat(nn.Module):
    """Concatenate a list of tensors along dimension."""

    def __init__(self, norm_channels, dimension=1):
        """Concatenates a list of tensors along a specified dimension."""
        super().__init__()
        self.d = dimension
        self.norm = LayerNorm(norm_channels, eps=1e-6, data_format="channels_first")
...
if __name__ == '__main__':
    # custom test
    # model = timm.create_model('unireplknet_l', num_classes=4, in_22k_pretrained=True)
    # model = UniRepLKNet(depths=UniRepLKNet_S_B_L_XL_depths, dims=(192, 384, 768, 1536), pretrained=False, num_classes=4)
    model = UniRepLKNet(depths=UniRepLKNet_T_depths, dims=(80, 160, 320, 640), pretrained=False, num_classes=None)
    data = torch.randn(1, 3, 640, 640).cuda(1)
    out = model.cuda(1)(data)
    data_list = [data] * 30
    for i in tqdm(data_list, desc='speed test'):
        out = model.cuda(1)(i)    # 591GFLOPs, 218M param, 4FPS, occupy 12G memory
    print(out.shape)
