...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # self.cv2 = nn.ModuleList(nn.Sequential(Conv(x, 64, 3), Conv(64, 64, 3), nn.Conv2d(64, 4 * self.reg_max, 1)) for x in ch)
...
class M_Detect(nn.Module):  # M表示modified，在这里修改自己想要的head
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2.append(nn.Sequential(Conv(ch[0], 64, 3), Conv(64, 64, 3), nn.Conv2d(64, 52, 1)))
        self.cv2.append(nn.Sequential(Conv(ch[1], 64, 3), Conv(64, 64, 3), nn.Conv2d(64, 64, 1)))
        self.cv2.append(nn.Sequential(Conv(ch[2], 64, 3), Conv(64, 64, 3), nn.Conv2d(64, 64, 1)))
        self.different_channel = True   # NOTE 这个属性不要删掉，改成True或None
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
    ...
    def process_tensor(self, pred_dist):
        if pred_dist.shape[-1] == 16:
    ...
    def self_decode_2(self, pred_dist):
